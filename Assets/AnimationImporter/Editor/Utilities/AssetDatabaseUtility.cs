	...
	public static class AssetDatabaseUtility
	{
		...
		public static string projectPath
		{
			...
			get
			{
				...
				return Application.dataPath.RemoveLastLetters(Path.DirectorySeparatorChar + "Assets".Length);
			}
		}
		...
		{
			var pathDirectory = Path.GetDirectoryName(unityFilePath) + Path.DirectorySeparatorChar;
		...
		private static void CreateDirectoriesInPath(string unityDirectoryPath)
		{
			...
			if (unityDirectoryPath[unityDirectoryPath.Length - 1] != Path.DirectorySeparatorChar)
			{
			...
			var folders = unityDirectoryPath.Split(Path.DirectorySeparatorChar);
			...
			foreach (var folder in folders)
			{
				...
				pathToFolder = string.Concat(pathToFolder, Path.DirectorySeparatorChar);
			}
		}
